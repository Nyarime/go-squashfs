...
// Pure Go XZ/LZMA2/LZMA decompressor — replaces github.com/ulikunitz/xz
// Implements: XZ container, LZMA2 wrapper, LZMA range decoder + LZ77
...
func (lr *lzma2Reader) decodeChunk() error {
	...
	// LZMA chunk
	// Bit 7: always 1 (LZMA chunk)
	// Bit 6: Reset dictionary
	// Bit 5: Reset state and read new properties
	// Bits 4-0: bits 20-16 of uncompressed size
	needProps := false
	needStateReset := false
	needDictReset := false

	if c&0x20 != 0 {
		// Bit 5 set: reset state + new properties
		needStateReset = true
		needProps = true
	}
	if c&0x40 != 0 {
		// Bit 6 set: reset dictionary
		needDictReset = true
		needStateReset = true
		needProps = true
	}
	...
	if needProps {
		...
		//compSize-- // prop byte is NOT part of compressed size in LZMA2
...
func decodeLzmaProps(b byte) (lc, lp, pb uint32) {
	pb = uint32(b) / 45
	rem := uint32(b) - pb*45
	// wait, the encoding is: b = pb * 45 + lp * 9 + lc
	// Actually: b = (pb * 5 + lp) * 9 + lc
	lc = uint32(b) % 9
	rem = (uint32(b) / 9)
	lp = rem % 5
	pb = rem / 5
...
func xzNewReaderImpl(r io.Reader) (*xzReader, error) {
	...
	xr := &xzReader{r: r, checkType: checkType}
	return xr, nil
}
...
func (xr *xzReader) readBlockHeader() error {
	...
	// Read filters
	if numFilters != 1 {
		return fmt.Errorf("xz: expected 1 filter, got %d", numFilters)
	}
